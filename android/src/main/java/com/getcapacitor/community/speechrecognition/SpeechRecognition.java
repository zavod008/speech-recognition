...
public class SpeechRecognition extends Plugin implements Constants {
      ...
      .post(
        new Runnable() {
          ...
          public void run() {
            ...
            Logger.debug("Instantiated SpeechRecognizer in load()");
          }
        }
      );
  }
  ...
  public void available(PluginCall call) {
    Logger.info("Called for available(): %b", isSpeechRecognitionAvailable() ? "yes" : "no");
  ...
  private void beginListening(
    ...
    Logger.debug("Beginning to listen for audible speech");
    ...
    if (showPopup) {
      ...
      startActivityForResult(call, intent, "processListening");
    } else {
      bridge
        .getWebView()
        .post(
  ...
  @ActivityCallback
  public void processListening(PluginCall call, ActivityResult result) {
    if (call == null) {
      return;
    }
    ...
    if (result.getResultCode() == Activity.RESULT_OK) {
      try {
        matchesList = data.getStringArrayListExtra(
          RecognizerIntent.EXTRA_RESULTS
        );
        JSObject object = new JSObject();
        object.put("matches", new JSArray(matchesList));
        call.resolve(object);
      } catch (Exception ex) {
        call.reject(ex.getMessage());
      }
